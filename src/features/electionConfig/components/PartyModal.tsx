...
const PartyModal: React.FC<PartyModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      if (editingParty) {
        ...
        const initialColors =
          editingParty.colors && editingParty.colors.length > 0
            ? editingParty.colors
  ...
  const handleColorChange = (index: number, value: string) => {
    ...
    if (hex.length === 7 && !validateHex(hex)) {
      setErrors((prev) => ({
        ...prev,
        colors: "Formato inválido en uno de los colores",
      }));
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const normalizedColors = colors
      ...
      .filter((color) => validateHex(color));
    const effectiveReferendumColors =
      normalizedColors.length > 0
        ? normalizedColors
        : [editingParty?.colorHex || DEFAULT_COLOR];

    if (!name.trim()) newErrors.name = "El nombre es obligatorio";
    if (!isReferendum && colors.some((c) => !validateHex(c)))
      newErrors.colors = "Formato de color inválido";
    if (!isReferendum && !logoBase64 && !logoPreview) {
      newErrors.logo = isReferendum
    ...
    try {
      await onSave({
        name: name.trim(),
        colors: isReferendum ? effectiveReferendumColors : colors,
        colorHex: isReferendum ? effectiveReferendumColors[0] : colors[0],
        logoBase64: isReferendum ? undefined : logoBase64 || logoPreview,
      });
    } catch {}
  };
        ...
        <div>
          ...
          <div className="space-y-3">
            {colors.map((color, index) => (
              <div
                key={index}
                className="grid gap-3 sm:grid-cols-[4rem_3rem_minmax(0,1fr)_2rem_2rem] sm:items-center"
              >
                ...
                <div
                  ...
                  style={{
                    backgroundColor: validateHex(color) ? color : "#ccc",
                  }}
                />
          ...
          {isReferendum ? (
            ...
            <p className="mt-2 text-xs text-gray-400">
              Puedes agregar hasta 4 colores en total.
            </p>
          )}
        </div>
