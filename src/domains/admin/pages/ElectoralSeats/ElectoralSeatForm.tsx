...
const ElectoralSeatForm: React.FC = () => {
  ...
  const id = pathname.match(/\/asientos-electorales\/editar\/([^/]+)/)?.[1];
  ...
  type ElectoralSeatFormValues = typeof initialValues;
  ...
  const handleSubmit = async (values: {
    ...
    try {
      ...
      setIsModalOpen(true);
      pushBrowserUrl({ pathname: '/asientos-electorales' });
    } catch (err) {
      ...
      <div className="p-6 bg-slate-50 min-h-screen">
        <div className="w-full max-w-5xl mx-auto p-8 bg-white rounded-lg shadow-lg">
          ...
          <Formik
            ...
            enableReinitialize
          >
            {({ isSubmitting, setFieldValue }: FormikProps<ElectoralSeatFormValues>) => (
