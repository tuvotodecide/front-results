...
const ProvinceForm: React.FC = () => {
  ...
  const id = pathname.match(/\/provincias\/editar\/([^/]+)/)?.[1];
  ...
  type ProvinceFormValues = typeof initialValues;
  ...
  const handleSubmit = async (values: {
    ...
    try {
      ...
      setIsModalOpen(true);
      pushBrowserUrl({ pathname: '/provincias' });
    } catch (err) {
      ...
      <div className="p-6 bg-slate-50 min-h-screen">
        <div className="w-full max-w-5xl mx-auto p-8 bg-white rounded-lg shadow-lg">
          ...
          <Formik
            ...
            enableReinitialize
          >
            {({ isSubmitting, setFieldValue }: FormikProps<ProvinceFormValues>) => (
              <Form className="space-y-6">
                ...
                <div className="flex justify-end space-x-4 pt-6 border-t border-gray-300">
                  <button
                    type="button"
                    onClick={() => pushBrowserUrl({ pathname: '/provincias' })}
