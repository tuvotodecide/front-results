...
import type { CreateDepartmentType } from '@/types';
...
const DepartmentForm: React.FC = () => {
  ...
  const id = pathname.match(/\/departamentos\/editar\/([^/]+)/)?.[1];
  ...
  const handleSubmit = async (values: { name: string; active: boolean }) => {
    try {
      ...
      if (isEditMode && id) {
        ...
        await createItem(payload as CreateDepartmentType).unwrap();
      }
      setIsModalOpen(true);
      pushBrowserUrl({ pathname: '/departamentos' });
    } catch (err) {
