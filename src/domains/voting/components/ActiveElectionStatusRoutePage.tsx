...
import type { ConfigStep, PartyWithCandidates, Voter } from "@/features/electionConfig/types";
import {
...
const canEditSchedule = (event?: { state?: string | null; status?: string | null; votingStart?: string | null }) => {
    ...
    <div className="space-y-2 text-sm text-gray-600">
      {lines.map((line) => (
        <p key={`${line.label}-${line.value}`}>
          <span className="font-semibold text-gray-800">{line.label}:</span> <span>{line.value}</span>
        </p>
      ))}
    </div>
  </div>
...
export default function ActiveElectionStatusRoutePage({
  ...
  const { data: events = [] } = useGetVotingEventsQuery(tenantId ? { tenantId } : undefined, {
    skip: !tenantId,
  });
  const { data: roles = [], isLoading: loadingRoles } = useGetEventRolesQuery(actualElectionId, {
    skip: !actualElectionId,
  });
  const { data: options = [], isLoading: loadingOptions } = useGetEventOptionsQuery(
    actualElectionId,
    {
      skip: !actualElectionId,
    },
  ...
  const { data: padronData, isLoading: loadingPadronVoters } = useGetPadronVotersQuery(
  ...
  const parties = useMemo<PartyWithCandidates[]>(
    () => options.map(optionToPartyWithCandidates),
    [options],
  );
  const currentPadron = padronVersions.find((item) => item.isCurrent) ?? padronVersions[0];
    ...
    return (
      <div className="min-h-[calc(100vh-64px)] bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-500">Cargando información de la votación...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-gray-50 min-h-[calc(100vh-64px)]">
      <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
        ...
        {shouldShowResults && resultsData && (
          <div className="rounded-2xl border border-violet-200 bg-violet-50 p-5 shadow-sm">
            <div className="flex items-center justify-between gap-4">
              <div>
                <h2 className="text-lg font-bold text-violet-900">Resultados disponibles</h2>
              ...
              <button
                type="button"
                onClick={() => router.push(`/elections/${actualElectionId}/public`)}
        ...
        <div className="space-y-6">
          <ConfigStepsTabs currentStep={activeTab} completedSteps={[1, 2, 3]} onStepChange={setActiveTab} />
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          <div className="w-full space-y-5">
            <div>
              <h2 className="text-2xl font-bold text-gray-900">Papeleta Electoral</h2>
            ...
            {parties.length === 0 ? (
              ...
              <div className="grid gap-6 xl:grid-cols-2">
                {parties.map((party) => (
                  <div key={party.id} className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                    ...
                    <div className="p-6">
                      <div className="flex items-center gap-4 pb-5 border-b border-gray-200">
                        {party.logoUrl ? (
                          <img src={party.logoUrl} alt={party.name} className="h-16 w-16 rounded-xl object-cover border border-gray-200" />
                        ) : (
                        ...
                        {party.candidates.map((candidate) => (
                          <div key={candidate.id} className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <p className="text-xl font-semibold text-gray-800">{candidate.fullName}</p>
                            </div>
                          </div>
                        ))}
                      </div>
        ...
        {otherElections.length > 0 && (
          <div className="space-y-4">
            <h2 className="text-xl font-semibold text-gray-800">Otras votaciones</h2>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {otherElections.map((item) => (
                <button
                  type="button"
                  key={item.id}
                  onClick={() => router.push(buildElectionConfigPath(item.id, "status"))}
      ...
      >
        <div className="space-y-5">
          <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
            El backend solo permite editar el cronograma en borrador o cuando faltan al menos 24 horas para el inicio de la votación.
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <label className="block">
              ...
              <input
                ...
                onChange={(eventInput) => {
                  setScheduleForm((current) => ({ ...current, votingStart: eventInput.target.value }));
                  setScheduleError(null);
                  setScheduleSuccess(null);
                }}
            ...
            <label className="block">
              ...
              <input
                ...
                onChange={(eventInput) => {
                  setScheduleForm((current) => ({ ...current, votingEnd: eventInput.target.value }));
                  setScheduleError(null);
                  setScheduleSuccess(null);
                }}
                className="w-full rounded-lg border border-gray-300 px-4 py-3 text-sm text-gray-800 outline-none transition focus:border-[#459151] focus:ring-2 focus:ring-[#459151]/15"
              ...
              <input
                ...
                onChange={(eventInput) => {
                  setScheduleForm((current) => ({
                    ...current,
                    resultsPublishAt: eventInput.target.value,
                  }));
                  setScheduleError(null);
                  setScheduleSuccess(null);
                }}
          ...
          <p className="text-sm text-gray-500">
            El sistema no está enviando una notificación automática al padrón cuando este horario cambia. Si eso debe ser obligatorio, hay que agregarlo en backend.
          </p>
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  setScheduleError(getErrorMessage(error, "No se pudo actualizar el horario."));
                }
              }}
