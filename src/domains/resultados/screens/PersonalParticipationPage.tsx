...
import { Link, useNavigate } from "@/domains/resultados/navigation/compat";
import {
  ChevronDown,
  ChevronUp,
  FileText,
...
import useElectionConfig from "@/domains/resultados/hooks/useElectionConfig";
import useElectionId from "@/domains/resultados/hooks/useElectionId";
...
type DelegateActivityTableData = {
  tableCode?: string;
  tableNumber?: string;
  location?: string;
  ballotId?: string | null;
...
type ParticipationAttestationRow = {
  delegateDni: string;
  delegateName: string;
  location: string;
...
const ParticipacionPersonal: React.FC = () => {
  ...
  const attestationRows = useMemo(() => {
    const rows: ParticipationAttestationRow[] = [];
    ((tablesResp?.data ?? []) as DelegateActivityTableData[]).forEach((table) => {
      (table.attestationDetails ?? []).forEach((att) => {
  ...
  const delegadosSinVotar = useMemo(() => {
    ...
    return (delegatesResp.data as DelegateWithoutVoteRow[]).filter(
      (d) => d.totalAttestations === 0,
    );
  }, [delegatesResp?.data]);
    ...
    return (
      <StatusMessage
        ...
        action={{
          ...
          onClick: () => navigate("/resultados/login"),
        }}
        variant="info"
      />
    );
              ...
              <div className="overflow-x-auto">
                ...
                <tbody className="divide-y divide-slate-50">
                  {tablesLoading ? (
                    ...
                    <>
                      {attestationRows.map((row, idx: number) => (
                        <tr
            ...
            <div className="bg-white rounded-lg shadow-xl overflow-hidden border border-red-200">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-separate border-spacing-0">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {delegadosSinVotar.map((d, idx: number) => (
                      <tr
                        key={`${d.dni ?? "sin-ci"}-${idx}`}
