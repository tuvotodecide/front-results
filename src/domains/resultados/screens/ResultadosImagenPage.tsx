...
import LocationSection from "../../../legacy-pages/Resultados/LocationSection";
import { useParams, useNavigate, useSearchParams } from "../navigation/compat";
import {
...
interface ImageVoteItem {
  partyId: string;
  votes?: number;
}

interface BallotAttestation {
  _id: string;
  support?: boolean;
  userRole?: string;
  userName?: string;
...
const BASE_NFT_URL =
  getRuntimeEnv("VITE_BASE_NFT_URL", "NEXT_PUBLIC_BASE_NFT_URL") || "";
...
const ResultadosImagen = () => {
  ...
  const { attestationsInFavor, attestationsAgainst } = useMemo(() => {
    ...
    const inFavor = attestationsData.filter(
      (attestation: BallotAttestation) => attestation.support === true,
    ).length;
                  ...
                  <div className="mt-6 border-t border-gray-100 pt-4">
                    ...
                    <div className="max-h-[300px] overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                      {attestationsData && attestationsData.length > 0 ? (
                        attestationsData.map((att: BallotAttestation) => (
                          <div
                            key={att._id}
