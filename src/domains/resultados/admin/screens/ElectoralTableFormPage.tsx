...
import { useNavigate, useParams } from '@/domains/resultados/navigation/compat';
...
const ElectoralTableForm: React.FC = () => {
  ...
  const [selectedMunicipality, setSelectedMunicipality] =
    useState<SelectOption | null>(null);
  const [selectedElectoralSeat, setSelectedElectoralSeat] =
    useState<SelectOption | null>(null);
  const [selectedElectoralLocation, setSelectedElectoralLocation] =
    useState<SelectOption | null>(null);
  ...
  const loadMunicipalities = async (inputValue: string = '') => {
    try {
      let data: Array<MunicipalityByProvinceType | MunicipalitiesType>;
      if (selectedProvince) {
    ...
    <>
      <div className="p-6 bg-slate-50 min-h-screen">
        <div className="w-full max-w-5xl mx-auto p-8 bg-white rounded-lg shadow-lg">
          ...
          <Formik<ElectoralTableFormValues>
          ...
          >
            {({
              isSubmitting,
              setFieldValue,
            }: FormikProps<ElectoralTableFormValues>) => (
              <Form className="space-y-6">
                ...
                <div className="flex justify-end space-x-4 pt-6 border-t border-gray-300">
                  <button
                    type="button"
                    onClick={() => navigate('/resultados/mesas')}
