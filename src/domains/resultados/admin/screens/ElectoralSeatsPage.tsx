...
const ElectoralSeats: React.FC = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const [searchParams, setSearchParams] = useState<Record<string, string>>({});
  const limit = 10;
  const { data } = useGetElectoralSeatsQuery({
    page: currentPage,
    limit,
    ...searchParams,
  });
  ...
  const [deleteItem] = useDeleteElectoralSeatMutation();
  const navigate = useNavigate();
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [itemToDelete, setItemToDelete] = useState<ElectoralSeatsType | null>(
    null
  );

  const handleEdit = (electoralSeat: ElectoralSeatsType) => {
    navigate(`/resultados/asientos-electorales/editar/${electoralSeat._id}`);
  };

  const handleDelete = (item: ElectoralSeatsType) => {
    ...
    <div className="p-6 bg-gray-100">
      <div className="w-full p-8 bg-white rounded shadow-md">
        <div className="flex justify-between items-center mb-6 border-b pb-4 border-gray-400">
          ...
          <Link
            to="/resultados/asientos-electorales/nuevo"
