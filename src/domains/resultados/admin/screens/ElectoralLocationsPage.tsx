...
import BackButton from "@/domains/resultados/components/BackButton";
import {
...
const columns: ColumnDef<ElectoralLocationsType>[] = [
  {
...
const ElectoralLocations: React.FC = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const [searchParams, setSearchParams] = useState<Record<string, string>>({});
  const limit = 10;
  const { data } = useGetElectoralLocationsQuery({
    page: currentPage,
    limit,
    ...searchParams,
  });
  ...
  const [deleteItem] = useDeleteElectoralLocationMutation();
  const navigate = useNavigate();
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [itemToDelete, setItemToDelete] =
    useState<ElectoralLocationsType | null>(null);

  const handleEdit = (electoralLocation: ElectoralLocationsType) => {
    navigate(`/resultados/recintos-electorales/editar/${electoralLocation._id}`);
  };

  const handleDelete = (item: ElectoralLocationsType) => {
    ...
    <div className="p-6 bg-gray-100">
      <div className="w-full p-8 bg-white rounded shadow-md">
        <div className="flex justify-between items-center mb-6 border-b pb-4 border-gray-400">
          ...
          <Link
            to="/resultados/recintos-electorales/nuevo"
