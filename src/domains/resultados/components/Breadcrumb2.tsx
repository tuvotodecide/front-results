...
interface BreadcrumbFetchResponse {
...
type BreadcrumbQueryMap = Record<string, string>;
...
const Breadcrumb = ({ autoOpen = true }: Breadcrumb2Props) => {
  ...
  useEffect(() => {
    ...
    if (!isInitialized && paramsSnapshot.size > 0 && selectedPath2.length === 0) {
      ...
      ) as BreadcrumbQueryMap;

      const promises = fetchers
        .filter((f) => Boolean(paramsEntries[f.key]))
        .map((f) => {
          const id = paramsEntries[f.key];
          return f.fetch(id).then((resp: BreadcrumbFetchResponse) => ({
            index: f.index,
            key: f.key,
            data: resp.data,
          }));
