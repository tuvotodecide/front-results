...
const Register: React.FC = () => {
  ...
  const registerUser = async (
    ...
    try {
      ...
      // En registro SIEMPRE empieza por verificación de correo
      localStorage.setItem("pendingReason", "VERIFY_EMAIL");
      // Registro ok => pendiente (pero OJO: ahora pendiente es:
      // 1) debe verificar correo
      // 2) luego aprobación superadmin
      navigate("/pendiente", { replace: true });
    } catch (error: any) {
      const msg = error?.data?.message;
      if (Array.isArray(msg)) setServerErrors(msg);
      else if (typeof msg === "string") setServerErrors([msg]);
      else setServerErrors(["No se pudo registrar"]);
    } finally {
