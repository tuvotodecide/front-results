describe("Resultados - Casos Visuales Parte 2 (21-47)", () => {
    ...
    it("1: Búsqueda de mesa válida muestra resultados", () => {
        const tableData = {
            tableCode: "MESA-123",
            department: "La Paz",
            province: "Murillo",
    ...
    it("2: Manejo de Mesa/Imagen no Encontrada (404) muestra mensaje controlado", () => {
    ...
    it("3: Normalización de entrada (trim) navega con ID limpio", () => {
    ...
    it("4: Visualización IPFS + fallback si la imagen falla (onError)", () => {
        const id = "BALLOT-OK-1";
        cy.mockBallotById(id, require("../fixtures/ballot_ok.json"));
        cy.mockAttestationsByBallotId(id, require("../fixtures/attestations_ok.json"));
        cy.visitResultadosImagen(id);
        cy.wait("@ballotById");
        cy.wait("@attestationsByBallot");
        cy.get('[data-cy="ipfs-image-link"]').should("have.attr", "href").and("include", "https://ipfs.io/ipfs/");
        cy.get("body").then(($b) => {
    ...
    it("5: Integridad de enlaces de auditoría (Ver NFT / Metadata) tienen URLs correctas", () => {
        const id = "BALLOT-OK-1";
        cy.mockBallotById(id, require("../fixtures/ballot_ok.json"));
        cy.mockAttestationsByBallotId(id, require("../fixtures/attestations_ok.json"));
        cy.visitResultadosImagen(id);
        cy.wait("@ballotById");
        cy.get('[data-cy="link-nft"]').should("have.attr", "href").and("match", /^https?:\/\//);
        cy.get('[data-cy="link-metadata"]').should("have.attr", "href").and("match", /^https?:\/\//);
    });

    it("6: Recuperación de estado vía URL (deep link)", () => {
    ...
    it("7: Sincronización con historial del navegador (back/forward)", () => {
        cy.mockGeographyByIdBasic();
        cy.mockLiveResults({
            presidential: { delayMs: 0 },
            deputies: { delayMs: 0 },
        ...
        cy.get('[data-cy="department-select"]', { timeout: 10000 }).should("contain.text", "La Paz");
        cy.get('[data-cy="province-select"]').should("contain.text", "Murillo");
    });

    it("8: Alcaldía incluye concejales (misma elección / misma selección territorial)", () => {
        const mayorResults = {
            results: [
    ...
    it("9: Filtros con valores no existentes (IDs inválidos) no crashean", () => {
    ...
    it("10: Listas grandes (performance UI) - 100+ opciones", () => {
    ...
    it("11: Renderizar correctamente imágenes de comparación", () => {
    ...
    it("41-b: Gobernación (solo Departamento carga resultados)", () => {
        ...
        cy.wait("@departments");
        cy.get('body').then(($body) => {
            const btns = $body.find('button, [type="button"]').filter(function(index, element) {
                return /aceptar|cerrar|entendido|ok/i.test(element.textContent || "");
            });
            if (btns.length) {
        ...
        cy.get('[data-cy="option-0-dep-lpz"]').click();
        cy.wait("@provincesByDepartmentLp");
        // Prevención: si la pantalla de resultados está desactivada, no esperar el request
        cy.get('body').then(($body) => {
            if ($body.find('[data-cy="presidential-results"]').length) {
    ...
    it("43: Cambiar tipo de elección cambia la lógica de filtros", () => {
        ...
        cy.visitResults();
        cy.get('body').then(($body) => {
            const btns = $body.find('button, [type="button"]').filter(function () {
                return /aceptar|cerrar|entendido|ok/i.test(this.textContent || "");
            });
            if (btns.length) {
                cy.wrap(btns.first()).click({ force: true });
            }
    ...
    it("46: Renderizar tabla de participaciones", () => {
        ...
        // Prevención: si la pantalla de resultados está desactivada, no esperar el request
        cy.get('body').then(($body) => {
            if ($body.find('[data-cy="statsbars"]').length) {
                cy.wait("@liveResultsPresidential");
