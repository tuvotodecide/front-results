...
import { AUTH_COOKIE_KEYS, handleResultadosAccess, handleVotacionAccess } from "../../middleware";
...
describe("MX-16 | acceso y shell Superadmin", () => {
  ...
  it("[MX-16][ADM-ACC-P0-001][INTEGRACION] renderiza el guard con una sesión global válida", () => {
  ...
  it("[MX-16][ADM-ACC-P0-001][ACEPTACION] hidrata la sesión dev y redirige rutas privadas de votación y resultados", async () => {
  ...
  it("[MX-16][ADM-ACC-P0-002][INTEGRACION] abre los destinos reales de la barra global", async () => {
  ...
  it("[MX-16][ADM-SEC-P0-001][INTEGRACION] muestra acceso restringido a una sesión institucional", () => {
    renderWithAuthStore(<SuperadminGuard><p>Datos globales</p></SuperadminGuard>, { ...globalAuth, role: "TENANT_ADMIN", activeContext: null, user: { ...globalAuth.user, role: "TENANT_ADMIN" } });
    expect(screen.getByText("Acceso restringido")).toBeInTheDocument();
    expect(screen.queryByText("Datos globales")).not.toBeInTheDocument();
  });

  it("[MX-16][ADM-SEC-P0-001][ACEPTACION] redirige visitante de ruta global al login", async () => {
