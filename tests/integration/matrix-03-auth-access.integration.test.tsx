...
describe("MX-03 | auth y acceso | integraciones canónicas", () => {
  ...
  it("[MX-03][AUT-LOG-P0-005][INTEGRACION] vuelve a la ruta protegida pedida tras iniciar sesión en votación", async () => { const user = userEvent.setup(); router.params = new URLSearchParams("from=/votacion/elecciones/e-1/config/general"); router.login.mockReturnValue({ unwrap: vi.fn().mockResolvedValue({ accessToken: "token", role: "TENANT_ADMIN", active: true, availableContexts: [{ type: "TENANT", tenantId: "tenant-1" }], defaultContext: { type: "TENANT", tenantId: "tenant-1" }, user: { id: "tenant", email: "tenant@test.com", name: "Tenant", role: "TENANT_ADMIN", active: true } }) }); const { container } = renderWithAuthStore(<LoginVotacionPage />); await user.type(container.querySelector('[data-cy="login-email"]') as HTMLInputElement, "tenant@test.com"); await user.type(container.querySelector('[data-cy="login-password"]') as HTMLInputElement, "12345678"); await user.click(container.querySelector('[data-cy="login-submit"]') as HTMLButtonElement); await waitFor(() => expect(router.navigate).toHaveBeenCalledWith("/votacion/elecciones/e-1/config/general", { replace: true })); });
  it("[MX-03][AUT-LOG-P0-005][INTEGRACION] ignora un from externo y usa el inicio del contexto en votación", async () => { const user = userEvent.setup(); router.params = new URLSearchParams("from=https://evil.test/votacion/elecciones"); router.login.mockReturnValue({ unwrap: vi.fn().mockResolvedValue({ accessToken: "token", role: "TENANT_ADMIN", active: true, availableContexts: [{ type: "TENANT", tenantId: "tenant-1" }], defaultContext: { type: "TENANT", tenantId: "tenant-1" }, user: { id: "tenant", email: "tenant@test.com", name: "Tenant", role: "TENANT_ADMIN", active: true } }) }); const { container } = renderWithAuthStore(<LoginVotacionPage />); await user.type(container.querySelector('[data-cy="login-email"]') as HTMLInputElement, "tenant@test.com"); await user.type(container.querySelector('[data-cy="login-password"]') as HTMLInputElement, "12345678"); await user.click(container.querySelector('[data-cy="login-submit"]') as HTMLButtonElement); await waitFor(() => expect(router.navigate).toHaveBeenCalledWith("/votacion/elecciones", { replace: true })); expect(router.navigate).not.toHaveBeenCalledWith(expect.stringContaining("evil.test"), expect.anything()); });
